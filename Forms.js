...
const form = document.querySelector("form"),
        nextButton = form.querySelector(".Next-Button"),
        backButton = form.querySelector(".Back-Button"),
        combinedInput = form.querySelectorAll(".first input");

nextButton.addEventListener("click", ()=> {
    combinedInput.forEach(input => {
        if(input.value != ""){
...
window.onload = function () {
    ...
    provinceSelection.onchange = (e) =>{
        ...
        for (let i=0; i < city.length; i++){
            citySelection.options[citySelection.options.length] = new Option(city[i], city[i])
        }  
    }
    
};

//Uploading file on PDF appearing on right side
...
// Change format of phone
function formatChange(value) {
    if (!value) return value;
    const contactNum = value.replace(/[^\d]/g, '');
    const contactNumLength = contactNum.length;
    if (contactNumLength < 5) return contactNum;
    if (contactNumLength < 8) {
      return `${contactNum.slice(0, 4)}-${contactNum.slice(4)}`;
    }
    return `${contactNum.slice(0, 4)}-${contactNum.slice(
      4,
      7
    )}-${contactNum.slice(7, 10)}`;
  }

  function numFormatChange() {
    const idphone = document.getElementById('phone-num');
    const formattedInputValue = formatChange(idphone.value);
    idphone.value = formattedInputValue;
  }